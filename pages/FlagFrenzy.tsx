    ...
    <div className="h-[100dvh] bg-surface flex flex-col p-4 overflow-hidden font-sans">
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full flex flex-col min-h-0 bg-white rounded-3xl border border-gray-100 p-6 md:p-10 overflow-hidden relative">
         <div className="flex flex-col items-center justify-center flex-1 min-h-0 pb-4 overflow-hidden">
            <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-2 shrink-0">Analyze National Flag</p>
            <div className="flex-1 flex items-center justify-center overflow-hidden w-full min-h-[100px]">
              <span className="text-8xl md:text-[10rem] select-none filter drop-shadow-xl transform transition-transform duration-700">{currentQuestion.country.flag}</span>
            </div>
         </div>
         <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 shrink-0">
            {currentQuestion.options.map((option) => {
              ...
              return (
                <button key={option.id} onClick={() => handleAnswer(option.name)} disabled={!!selectedAnswer} className={`relative p-5 rounded-2xl font-display font-bold text-lg transition-all duration-200 flex items-center justify-center min-h-[72px] ${stateStyles}`}>
